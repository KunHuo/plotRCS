plot_RCS <- function(data, event = NULL, time = NULL, exposure = NULL, covariates = NULL, positive = NULL,
                          knots = knots3(),
                          ref.knot = 1,
                          ref.value = NULL,
                          color = "red",
                          ...
                          font.size = 11, font.family = "serif", line.size = 0.25,  line.color = "black", aspect.ratio = 0.8, ...){


  if(inherits(data, "task.surv")){
    if(is.null(event)){
      event <- data$event
    }
    if(is.null(time)){
  ...
  if(!is.null(positive)){
    ...
    data[[event]] <- ifelse(data[[event]] == positive, 1, 0)
  }
  ...
  if(is.null(time)){
    formula <- sprintf("%s ~ rms::rcs(%s, m_)", event, exposure)
    ...
    formula <- sprintf("survival::Surv(%s, %s) ~ rms::rcs(%s, m_)", time, event, exposure)
    if(length(covariates) != 0){
      formula <- paste(formula, paste(covariates, collapse = " + "), sep = " + ")
    }
    formula <- stats::as.formula(formula)
  ...
  if(exists("ddist_")){
    rm("ddist_", inherits = TRUE, envir = parent.frame())
  }

  if(exists("m_")){
    rm("m_", inherits = TRUE, envir = parent.frame())
  }
